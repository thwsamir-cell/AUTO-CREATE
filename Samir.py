...
#___ OPEN SOURCE : SAMIR DAHIT

#____ OWNER : SAMIR HERO
# pip install pyotp  
...
#______
os.system("https://samirdahit-cyber.github.io/Samir.Com")
#-----------------------( STYLE )-----------------------#
...
def getKey():
    ...
    z="X".join(x)
    keys=z[15:]
    return "SAMIR-ERAXX-"+keys
final_key=getKey()
#-----------------------( FILE KEY )-----------------------#
file_path='/data/data/com.termux/files/usr/bin/.Samir.txt'
try:
...
def ethanproxy():
    ...
    return None
#-----------------------( SYS )-----------------------#
sys.stdout.write('\x1b[1;37m\x1b]2;📱<SAMIR-DAHIT>💻\x07')
#-----------------------( FILE-PATH )-----------------------#
sd_folder="/sdcard/SAMIR-AUTO"
Samir_folders=("AUTO","2FA","COKI")
os.makedirs(sd_folder,exist_ok=True)
for folder in Samir_folders:
...
surnames3=["Adhikari","Acharya","Aryal","Awasthi","Bajgain","Baniya","Bastola","Bhattarai","Bhandari","Bhusal",
    ...
    "Chy","Chaudhary","Rokaya","Shah","Sen","Shrestha","Sharma","Subedi","Thapa","Adhikari",
...
surnames4=["Adhikari", "Acharya", "Aryal", "Awasthi", "Bajgain", "Baniya",
    ...
    "Chy", "Chaudhary", "Bhattachan", "Poon", "Jomsom",
...
os.system("xdg-open https://samirdahit-cyber.github.io/Samir.Com")
#-----------------------( LOGO )-----------------------#
logo = """   \033[1;31m
███████╗ █████╗ ███╗   ███╗██╗██████╗ ███████╗██╗   ██╗
██╔════╝██╔══██╗████╗ ████║██║██╔══██╗██╔════╝╚██╗ ██╔╝
...
╚══════╝╚═╝  ╚═╝╚═╝     ╚═╝╚═╝╚═╝  ╚═╝╚══════╝   ╚═╝"""
#-----------------------( INFO )-----------------------#
info=(f"""{xp} AUTHOR    {xpxxx} {green}SAMIR-DAHIT 
{xp} FACEBOOK  {xpxxx} {green}SAMIR
{xp} TYPE      {xpxxx} {green}AUTO CREATE FACEBOOK
{xp} OWNER    {xpxxx} {green} SAMIR HERO 
{xp} STATUS    {xpxxx} {green}PAIDxFREE
...
def __AUTOX__():
    ...
    for _ in range(int(__NUM__)):
        try:
            ...
            sys.stdout.write(f'\r {white}[{green}SAMIR-CREATE{white}]-{white}[{green}OK:-%s{white}] '%(len(oks)));sys.stdout.flush()
            sys.stdout.flush()
            ...
            if "c_user" in login_coki:
                coki="sb=Cracked.By-SAMIR-ERAXX_Tool;"+";".join([f"{key}={value}" for key,value in login_coki.items()])
                ...
                if show_details=='y':
                    ...
                    print(f"\r{xp} DOB      {xpxxx} {payload['birthday_day']}-{payload['birthday_month']}-{payload['birthday_year']}\033[1;37m")
                    print(f"\r{xp} UID      {xpxxx} {uid}\033[1;37m")
                    print(f"\r{xp} PASS     {xpxxx} {pww}\033[1;37m")
                    print(f"\r{xp} COOKIE   {xpxxx} {coki}\033[1;37m")
                    __LINE__()
                else:
                    print(f'\r{green} [SAMIR-OK] '+uid+' | '+pww+'\033[1;97m')
                open('/sdcard/SAMIR-AUTO/AUTO/SAMIR-AUTO-OK.txt', 'a').write(uid+'|'+pww+'|'+coki+'\n')
                oks.append(uid)
